...
bool x = 0; 
unsigned long timer ; 
void giro_Levl_tilt(int timer_foo, int frequen_foo){
  if(millis() - timer > timer_foo){
    if(x == 1){digitalWrite(led,1);tone(buzz, frequen_foo);x=0;}
    if(x == 0){digitalWrite(led1,1);tone(buzz, frequen_foo);x=1;}
    timer = millis(); 
  }
  else {
    digitalWrite(led,0);
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  if(angle_x < -20 || angle_x > 20 || angle_y > 20 || angle_y < -20){giro_Levl_tilt(500, 1800);
   if (angle_x < -30 || angle_x > 30|| angle_y > 30 || angle_y < -30){giro_Levl_tilt(200, 1500);
     if (angle_x < -40 || angle_x > 40|| angle_y > 40 || angle_y < -40){giro_Levl_tilt(100, 1000);
      if (angle_x < -50 || angle_x > 50|| angle_y > 50 || angle_y < -50){giro_Levl_tilt(5, 500);}}}}
  else{digitalWrite(led,0);digitalWrite(led1,0);noTone(buzz);}
